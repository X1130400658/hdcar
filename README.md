...
public class SegementProcessorZipkin {
    ...
    @Autowired
    private SpanTransformerSupplier spanTransformerSupplier;

    @Value("${kafka.zipkin.topic}")
    ...
    private void doAggregateTracingTo(KStream<String, List<SpanDto>> source) {
        ...
        transform.transform(spanTransformerSupplier);
    }

    /**
     * 判断span对是否需要处理
     *
     * @param spanDtoList span对
     * @return
     */
    public boolean isNeedHandle(List<SpanDto> spanDtoList) {
        for (SpanDto spanDto : spanDtoList) {
            if (StringUtils.isEmpty(spanDto.getServiceHostName())) {
                LOGGER.info("span name is null, no need to handle. span id is {}.", spanDto.getId());
                return false;
            }
        }
        if ((isClient(spanDtoList.get(0)) && isServer(spanDtoList.get(1)))
                || (isClient(spanDtoList.get(1)) && isServer(spanDtoList.get(0)))) {
            return true;
        }
        LOGGER.info("ss or cc, no need to handle. span id is {}.", spanDtoList.get(0).getId());
        return false;
    }
     ...
     */
    @Scheduled(fixedRate = Constants.CLEAR_CACHE_SPAN_INTERVAL_MS)
    public void clearCacheSpan() {
        LOGGER.info("start to clear CacheSpan.");
        ...
        for (Map.Entry<Object, Object> mapEntry : map.entrySet()) {
            ...
            // 清理缓存中留存时间大于两倍定时任务的数据
            if (currTimestamp - spanDto.getDealtime() > 2 * Constants.PROCESS_CACHE_SPAN_INTERVAL_MS) {
                LOGGER.info("delete CacheSpan {}.", mapEntry.getKey());
                redisUtil.delHash(Constants.SPAN_NAME_FLAG, (String) mapEntry.getKey());
            }
        }
    }
    ...
    public void processCacheSpan() {
        LOGGER.info("start to handle ss or cc. time is {}.", System.currentTimeMillis());
            ...
            try {
                ...
                for (int i = 0; i < spanDtoList.size() - 1; i++) {
                    for (int j = i + 1; j < spanDtoList.size(); j++) {
                        ...
                        if (spanArr != null && startTimestamp - spanArr[0].getDealtime() > Constants.CLEAR_CACHE_SPAN_INTERVAL_MS
                                && startTimestamp - spanArr[1].getDealtime() > Constants.CLEAR_CACHE_SPAN_INTERVAL_MS) {
        ...
        LOGGER.info("end to handle ss or cc. time is {}.", System.currentTimeMillis());
    }
    ...
    public void addTracingTo(List<SpanDto[]> spanArrList) {
        ...
        for (SpanDto[] spanArr : spanArrList) {
            ...
            if (isClient(spanArr[0])) {
                ...
                redisUtil.delHash(Constants.SPAN_NAME_FLAG, spanArr[0].getId());
            } else {
                timeStamp = spanArr[1].getTimestamp();
                annotations = spanArr[1].getAnnotations();
                redisUtil.delHash(Constants.SPAN_NAME_FLAG, spanArr[1].getId());
            }
            for (Annotation annotation : annotations) {
                switch (annotation.getValue()) {
                    case Constants.CLIENT_SENT:
