  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">交易照片</h4>
      </div>
...
window.onload=function(){
	...
	if(status=="1"){
	   ...
	   $.ajax({
				  type: 'get',
				  url: 'car/checkDeposit/'+pageNum+'/'+pageSize,
				  async: false,
				  data:  '',
...
function xuanran(datalist){	
       ...
       for(var i=0;i<getJsonLength(datalist);i++)
       {
		...
		var reg=regdate.toLocaleDateString();
			 tmp+='<tr><td>'+datalist[i].modelnote+'</td><td><a href="javascript:;" class="thumbnail"><img src="http://hdcarfiles-1251224234.cossh.myqcloud.com/listingfiles/'+datalist[i].listseq+'.png" alt="..."></a></td><td>'+date+'</td>'+
			'<td>'+reg+'</td><td>'+datalist[i].mileage+'</td><td>'+datalist[i].enginesize+'</td><td>'+datalist[i].price+'</td><td><a href="jsp/depositNote.jsp"  class="thumbnail" onclick="return getId('+datalist[i].listseq+')">交易记录</a>'+
			'<a id="'+datalist[i].listseq+'" href="javascript:;" data-toggle="modal" data-target="#myModal" class="thumbnail" onclick="return check('+datalist[i].listseq+')">交易凭证</a></td></td><td><a href="car/checkDepositPass?listseq='+datalist[i].listseq+'" onclick="return ss()" class="thumbnail">通过</a>'+
			'<a href="car/checkDepositUnpass?listseq='+datalist[i].listseq+'" onclick="return ff()" class="thumbnail">不通过</a></td></tr>'
       }
...
function getId(id){
  ...
  function ff(){
    ...
    carid=id;
    $('.modal-body').html("");
    var image='<img src="http://hdcarfiles-1251224234.cossh.myqcloud.com/prooffiles/T'+id+'_0.png"/>';
    ...
    $("#next").click(function(){
    		 	...
    		 	$(".modal-body").html("");
    		 	name=carid+'_'+t;
    		 	 image='<img src="http://hdcarfiles-1251224234.cossh.myqcloud.com/prooffiles/T'+name+'.png" onerror="alert(404)"/>';
   				$(".modal-body").append(image);
    	 });
    	...
    	$("#last").click(function(){
    		 	...
    		 	$(".modal-body").html("");
    		 	name=carid+'_'+t;
    		 	 image='<img src="http://hdcarfiles-1251224234.cossh.myqcloud.com/prooffiles/T'+name+'.png" onerror="alert(404)" alt="tp"/>';
   				$(".modal-body").append(image);
